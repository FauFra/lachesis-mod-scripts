...
function usage() {
  echo "Usage: $0 rate variant #reps #duration_min kakfa_host"
  exit 1
}
...
./scripts/run.py ./fausto/single_test/config.yaml -d "$DURATION" -r "$REPS" --statisticsHost "$(hostname)" --kafkaHost "$KAFKA_HOST" -c "$DATE_CODE" --sampleLatency true
