...
./scripts/run.py ./fausto/scripts/templates/FlinkLinearRoadKafkaRT.yaml -d "$DURATION" -r "$REPS" --statisticsHost "$(hostname)" --kafkaHost "$KAFKA_HOST" -c "$DATE_CODE" --sampleLatency true
