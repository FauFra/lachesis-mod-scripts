...
sudo chrt -r 99 ./lachesis-mod-scripts/rt_scripts/start_parachute_rt.sh --mins $((($DURATION*$REPS*3*8)+240)) # 240 = additional 4 hours | 3 = OS, LACHESIS, LACHESIS-MOD | 8 = rate range 
trap "$PARACHUTE_RT" EXIT

./scripts/run.py ./lachesis-mod-scripts/scripts/templates/StormLinearRoadKafkaNiceRT.yaml -d "$DURATION" -r "$REPS" --statisticsHost "$(hostname)" --kafkaHost "$KAFKA_HOST" -c "$DATE_CODE" --sampleLatency true
