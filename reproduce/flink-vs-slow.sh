...
./scripts/run.py ./lachesis-mod-scripts/scripts/templates/FlinkVoipStreamKafkaSlow.yaml -d "$DURATION" -r "$REPS" --statisticsHost "$(hostname)" --kafkaHost "$KAFKA_HOST" -c "$DATE_CODE" --sampleLatency true
